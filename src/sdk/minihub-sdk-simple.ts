...
export class MiniHubSDK {
  ...
  async getEmployerProfileByAddress(employerAddress: string): Promise<EmployerProfile | null> {
    try {
      console.log('🔍 SDK: Searching for EmployerProfile for address:', employerAddress);
      ...
      if (registryObj.data && registryObj.data.content && registryObj.data.content.dataType === 'moveObject') {
        ...
        if (fields.employer_profiles && Array.isArray(fields.employer_profiles)) {
          for (const profileId of fields.employer_profiles) {
            try {
              const profileObj = await this.client.getObject({
                id: profileId,
                options: { showContent: true },
              ...
              if (profileObj.data && profileObj.data.content && profileObj.data.content.dataType === 'moveObject') {
                ...
                if (profileFields.employer_address === employerAddress) {
                  return {
                    id: profileFields.id?.id || profileObj.data.objectId,
                    employerAddress: profileFields.employer_address,
                    companyName: profileFields.company_name || '',
                    description: profileFields.description || '',
                    logoUrl: profileFields.logo_url || '',
                    website: profileFields.website || '',
                    industry: profileFields.industry || '',
                    employeeCount: Number(profileFields.employee_count || 0),
                    foundedYear: Number(profileFields.founded_year || 0),
                    createdAt: Number(profileFields.created_at || 0),
                    updatedAt: Number(profileFields.updated_at || 0),
                  };
                }
              ...
              console.log('⚠️ SDK: Error fetching employer profile object:', err);
            }
          }
        }
      }
      ...
      console.error('❌ SDK: Error fetching employer profile:', error);
      return null;
    }
  }

  /**
   * Get employer capabilities (EmployerCap objects) for a given address
   */
